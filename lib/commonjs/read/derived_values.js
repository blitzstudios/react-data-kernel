...
function createDerivedValues(ctx, def) {
  ...
  const lists = (0, _caches.createBoundedLru)(LISTS_MAX);
  ...
  const listKeyOf = (key, method, of) => `${(0, _args_key.cacheKeyOf)(parts(key))}${_args_key.KEY_SEP}${method}${_args_key.KEY_SEP}${(0, _args_key.stableKey)(of)}`;
  ...
  const overFilter = (key, extra) => {
    ...
    return keep(listKey, listed(resolve(key, members, extra), members), _caches.shallowEqualArray, at);
  };
  return {
    ...
    atEach: (key, ids) => ids.length ? keep(listKeyOf(key, 'atEach', ids), listed(resolve(key, ids), ids), _caches.shallowEqualArray) : NO_VALUES,
    pick: (key, ids) => {
      ...
      return keep(listKeyOf(key, 'pick', ids), out, _caches.shallowEqualRecord);
    },
